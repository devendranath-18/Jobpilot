...
function mcpToolToGroqTool(tool: any) {
  return {
   ...
   for (let step = 0; step < MAX_STEPS; step++) {
    ...
    try {
      ...
      if (err?.status === 400 && err?.error?.error?.code === "tool_use_failed") {
        console.log(`  [WARN] Groq rejected malformed tool call JSON at step ${step}. Retrying with a stronger instruction.`);
        messages.push({
          role: "system",
          content:
            "Your previous tool call had invalid JSON arguments — you used a comment or placeholder instead of the real data. You MUST inline the complete, literal JSON value for every argument. Never write comments like '/* see above */' or '/* placeholder */'. Retry the tool call now with full, real values.",
        });
        completion = await groq.chat.completions.create({
          model,
          messages,
...
console.log(`  [debug] step ${step}: finish_reason=${choice.finish_reason}, tool_calls=${message.tool_calls?.length || 0}, content="${message.content?.slice(0, 200)}"`);
  ...
  try {
    ...
    console.log(`  [WARN] ${toolName} produced invalid JSON arguments. Attempting cache-based recovery.`);
    console.log(`  [debug] raw arguments: ${toolCall.function.arguments}`);

    // Build args entirely from cache for tools where we already know
    // what real data should go in — bypass the LLM's broken JSON.
    if (toolName === "match_resume" && resultCache["parse_job_description"] && resultCache["parse_resume"]) {
      args = {
        parsed_jd: resultCache["parse_job_description"],
        parsed_resume: resultCache["parse_resume"],
      };
    } else if (toolName === "tailor_bullets" && resultCache["parse_resume"]) {
      args = {}; // your existing tailor_bullets block below fills this in fully
    } else if (toolName === "save_application") {
      args = {}; // your existing save_application block below fills this in fully
    } else {
      // No safe recovery possible — re-throw so it's visible, rather than
      // silently sending garbage to the tool.
      throw new Error(
        `Tool ${toolName} returned unparseable arguments and no cache-based recovery exists for it: ${parseErr}`
      );
    }
  }

  // ... rest of your existing substitution logic continues unchanged from here
     ...
     if (toolName === "tailor_bullets") {
          ...
          if (resultCache["parse_resume"]) {
            ...
            console.log("  [debug] jd nice_to_have_skills:", jd?.nice_to_have_skills);
                        ...
                        const relevantEntry = selectRelevantExperience(
              resultCache["parse_resume"].experience || [],
              jd?.required_skills || [],
  ...
  // If the underlying LLM call itself failed (rate limit, etc.), stop
  // immediately instead of letting garbage data cascade into every
  // downstream step with confusing, unrelated errors.
  if (resultText.includes("rate_limit_exceeded") || resultText.includes('"status":429')) {
    await client.close();
    throw new Error(
      `${toolName} failed because the underlying LLM call was rate-limited. Stopping now instead of continuing with bad data. Please wait a few minutes and try again.`
    );
  }
...
onProgress?.({ tool: toolName, status: "done" });
 ...
 await client.close();
    ...
    if (missingParts.length > 0) {
      throw new Error(
        `Agent finished but some steps didn't complete properly: ${missingParts.join(", ")}. This usually means a tool call failed silently or the model skipped a step. Please try again.`
      );
    }
