 ...
 * or NaN data.
 */
function validateInput(input: SaveApplicationInput): void {
  const errors: string[] = [];
...
export function saveApplication(
  ...
  const result = stmt.run(
    ...
    JSON.stringify(input.tailored_bullets),
    input.cover_letter
  );
